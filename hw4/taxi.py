...
class Agent():
    ...
    def choose_action(self, state):
        ...
        rand_num = np.random.uniform(1) # create a random number
        max_q = self.check_max_Q(state) # get the max Q of the state
        if(rand_num > self.epsilon): # if the random num > epsilon
            for i in range(6): # find the max Q's action and return the action
                if(self.qtable[state][i] == max_q):
                    return i
        else: # if the random num < epsilon
            return np.random.randint(6,size = 1) # return random action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        # TODO
        self.qtable[state][action] = self.qtable[state][action] - self.learning_rate * (self.qtable[state][action] - (reward + self.gamma * self.check_max_Q(next_state)))
        # the q learning
        # End your code
    ...
    def check_max_Q(self, state):
        ...
        return np.max(self.qtable[state]) # return the max value of the state
        # End your code
