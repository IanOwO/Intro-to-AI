...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        nparr = np.linspace(lower_bound,upper_bound,num_bins,endpoint=False) # create bin with lower bound
        nparr = np.delete(nparr,0) # delete lower bound
        return nparr # return bin
        # End your code
    ...
    def discretize_value(self, value, bins):
        ...
        # Begin your code
        # TODO
        return np.digitize(value,bins,right=True) # get the discretize value of the value
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        # TODO
        re = []
        for i in range(len(self.bins)): # for all bins, discretize the value by a given observation
            re.append(self.discretize_value(observation[i],self.bins[i]))
        return re # return a list of discretize values
        # End your code
    ...
    def choose_action(self, state):
        ...
        if(rand_num > self.epsilon): # if is larger then epsilon
            return np.argmax(self.qtable[tuple(state)]) # return the max q value's action
        else:
            return env.action_space.sample() # random choose a action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        # TODO
        if done: # if done, set max of next action's q to 0
            max_val = 0
        else: # else, get the max of next action's q
            max_val = np.max(self.qtable[tuple(next_state)])
        
        tmp = reward + self.gamma * max_val
        self.qtable[(tuple(state)+(action,))] = self.qtable[(tuple(state)+(action,))] - self.learning_rate * (self.qtable[(tuple(state)+(action,))] - tmp)
        # do q learning, with tuple as index
        # End your code
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        # TODO
        print("opt q:",(1-np.power(self.gamma,185))/(1-self.gamma))
        state = self.discretize_observation(self.env.reset()) # get the initial condition's discretize value
        return np.max(self.qtable[state[0]][state[1]][state[2]][state[3]]) # return the max q of the given observation
        # End your code
