...
class Agent():
    ...
    def learn(self):
        ...
        observations, actions, rewards, next_observations, done = self.buffer.sample(self.batch_size) # get the sample data
        state = torch.FloatTensor(np.array(observations)) # make it become a float tensor
        act = torch.LongTensor(actions).unsqueeze(1) # make it become a long tensor, squeeze the data to right size
        rwd = torch.FloatTensor(rewards) # make it become a float tensor
        next_state = torch.FloatTensor(np.array(next_observations)) # make it become a float tensor
        do = torch.BoolTensor(done) # make it become a bool tensor
        
        val = torch.gather(self.evaluate_net(state),1,act) # get the tensor of the act
        max_val = self.target_net(next_state).detach() * (~do).unsqueeze(-1) # calculate the next state's q max
        tar = rwd.unsqueeze(-1) + self.gamma * max_val.max(1)[0].view(self.batch_size,1)
        # calcculate the q at this state
        loss = nn.MSELoss()(val,tar) # to calculate the loss, make it get close to real q
 
        # calculate the new eval_net
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            # TODO
            rand_num = np.random.uniform(0,1) # a random probability
            if(rand_num > self.epsilon): # if is larger then epsilon
                action = torch.argmax(self.evaluate_net(torch.FloatTensor(state))).item()
                # return the max q value's action
                # it is a tensor, change state to tensor and put it in evaluate net, then get the max q's index
            else:
                action = env.action_space.sample() # random choose a action
            # End your code
        return action
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        # TODO
        print("opt q:",(1-np.power(self.gamma,200))/(1-self.gamma))
        ini = torch.FloatTensor(self.env.reset()) # get the initial float tensor
        state = ini.unsqueeze(0) # unsqueeze to right size
        return torch.max(self.target_net(state)).item() # return the max q of the tensor
