...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        cur = [] # to store WeakClassifier(feature)
        for i in range(len(features)): # go through all data in feature
          cur.append(WeakClassifier(features[i])) # change feature to weakclassifier 
        bestClf = None # define best classifier
        bestError = float('inf') # let the current best error be the max of float
        for i in range(len(cur)): # go through all weakclassifier
          errorTMP = 0 # the sum of current weakclassifier's error
          for j in range(len(iis)): # go through all images
            if cur[i].classify(iis[j]) != labels[j]:
              # check if the image's classify is same as the real label
              errorTMP += weights[j] # if not, add weight to the sum
          if errorTMP < bestError: # check if current error is smaller
            bestError = errorTMP # change the current best error num
            bestClf = cur[i] # change the current best classifier
